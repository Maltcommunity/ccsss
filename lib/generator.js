    ...
    return download(pageUrl, userAgent)
        .then(html => {
            if (html.toLowerCase().indexOf('<html') === -1) {
                throw new Error(`No HTML received, result is: ${html}`);
            }
...
async function callPenthouse(cfg, cssString, dimensions) {
    ...
    return new Promise((resolve, reject) => {
        penthouse({
            ...
            pageLoadSkipTimeout: cfg.pageLoadSkipTimeout || 0,
...
class CriticalCssGenerator extends EventEmitter {
    ...
    _unsafeProcessNextRequest() {
        ...
        return generateCriticalCss(cfg).then(criticalCss => {
            console.log(`Critical CSS generated for: ${cfg.url}, emit css generated event`);
            return this.emit('critical-css-generated', cfg, criticalCss);
        });
    }
