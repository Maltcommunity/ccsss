...
function fetchCss(pageUrl, dest, userAgent) {
        ...
        .then(css => {
            ...
            return new Promise((resolve, reject) => {
                fs.writeFile(dest, css, err => {
                    if (err) {
                        console.error(`Could not write CSS to ${dest}`);
                        reject(err);
                    } else {
                        resolve(true);
                    }
...
function generateCriticalCss(cfg) {
    const cssFile = tempfile('.css');
    let cleanCssFileAfterOperation = false;

    return fetchCss(cfg.url, cssFile, cfg.userAgent)
        .then(cssWritten => {
            if (!cssWritten) {
                // no need to call Penthouse
                return [];
            }
            cleanCssFileAfterOperation = true;
            const promisesForEachDimension = cfg.dimensions.map(dim => callPenthouse(cfg, cssFile, dim));
        ...
        .then(criticalCss => {
            if (cleanCssFileAfterOperation) {
                fs.unlink(cssFile, err => {
                    if (err) {
                        console.error(`Could unlink ${dest}`);
                    }
                });
            }
            return criticalCss;
