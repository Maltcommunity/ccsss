...
generator.on('critical-css-generated', (cfg, criticalCss) => {
    ...
    if (cfg.notificationUrl) {
        console.log(`Critical CSS generated, notifying: ${cfg.notificationUrl}`);
        const userAgent = cfg.userAgent || 'ccsss';
        httpRequest({
            method: 'POST',
...
function isValidGenerationRequest(cfg) {
    ...
    if (cfg.userAgent && typeof cfg.userAgent !== 'string') {
        return false;
    }
...
function acceptGenerationRequestIfValid(req, response) {
    ...
    req.on('end', () => {
        try {
            ...
            const resultEndpoint = 'http://' + req.headers.host + GENERATION_RESULT_PATH;
            cfg.resultEndpoint = resultEndpoint;

            const generationId = generator.enqueue(cfg);
